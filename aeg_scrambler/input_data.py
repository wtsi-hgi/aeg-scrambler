...
class CCLEExpression(InputData):
    ...
    def clean(self, config: Config) -> pd.DataFrame:
        ...
        self.data.drop(["depmapID", "primary_disease"], axis=0, inplace=True)
        ...
        self.data.drop_duplicates(
            keep=False, subset=["Gene_name"], inplace=True
        )
    ...
    def find_anomalous_score_of_gene_expression(self) -> None:
        ...
        self.data["Anomalous_score"] = self.data.apply(
            lambda gene: (gene["General_gene_expression"] - gene["Mean"])
            / gene["Std"],
            axis=1,
        )
...
class GeneAnnotations(InputData):
    ...
    def clean(self, config) -> None:
        ...
        self.data["Gene_biotype"] = (
            self.data["Attributes"]
            .str.extract('gene_biotype "(.*?)"', expand=False)
            .fillna("None")
        )
        ...
        self.data["Gene_name"] = (
            self.data["Attributes"]
            .str.extract('gene_name "(.*?)"', expand=False)
            .fillna("None")
        )
        ...
        self.data.drop(unwanted_cols, axis=1, inplace=True)
