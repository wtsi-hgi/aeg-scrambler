...
class Metrics:
    ...
    def merge_genetic_data(
        self, gene_annotations, ccle_expression, experimental_expression
    ):
        self.data = pd.merge(
    ...
    def find_gene_sizes(self):
        ...
        self.data["Gene_size"] = (
            self.data["Gene_end"] - self.data["Gene_start"]
        )
    ...
    def find_search_windows(self, config):
        ...
        self.data["Search_window_start"] = self.data.apply(
            ...
            if gene["Strand"] == "+"
            else gene["Gene_start"] - config.downstream_search,
            axis=1,
        )

        self.data["Search_window_end"] = self.data.apply(
            lambda gene: gene[downstream_search_start]
            + config.downstream_search
            if gene["Strand"] == "+"
            else gene[downstream_search_start] + config.upstream_search,
            axis=1,
        )
    ...
    def find_element_overlaps_within_search_window(self):
        ...
        self.overlaps = gene_search.intersect(
            elements_search, strandedness=False
        )
    ...
    def count_overlaps_per_gene(self):
        ...
        self.data = pd.merge(
            self.data,
            self.overlaps.groupby("Gene_name")
            .size()
            .reset_index(name=("Enhancer_count")),
            on="Gene_name",
            how="inner",
        )
    ...
    def find_nearby_enhancer_densities(self):
        ...
        summed_overlaps = (
            self.overlaps.loc[:, ["Gene_name", "Enhancer_proportion"]]
    ...
    def find_symmetry_of_elements(self):
        ...
        self.overlaps["Overlaps_upstream"] = self.overlaps.apply(
            lambda overlap: True
            if overlap["End"] <= overlap["Gene_start"]
            else False,
            axis=1,
        )

        self.overlaps_upstream = (
            self.overlaps.loc[:, ["Gene_name", "Overlaps_upstream"]]
            .groupby(["Gene_name"], as_index=False)["Overlaps_upstream"]
            .sum()
            .reset_index()
        )

        self.data = pd.merge(self.data, self.overlaps_upstream, on="Gene_name")

        self.data["Symmetry_ratio"] = 2 * (
            np.absolute(
                (self.data["Overlaps_upstream"] / self.data["Enhancer_count"])
                - 0.5
            )
        )
    ...
    def calculate_interest_score(self, config):
        ...
        scaled_genes = scaled_genes.assign(
            Interest_score=(
                ...
                * config.anomalous_expression_weight
                + scaled_genes["Enhancer_count"] * config.enhancer_count_weight
                ...
                * config.cell_line_expression_weight
                + scaled_genes["Gene_size"] * config.gene_size_weight
                + scaled_genes["Symmetry_ratio"] * config.symmetry_weight
            )
        ...
        self.data.sort_values("Interest_score", ascending=False, inplace=True)
    ...
    def export_gene_scores_report(self, config):
        ...
        report_path = f"{config.results_directory}gene_rankings.{id.lower()}.tsv"
        ...
        ].to_csv(report_path, sep="\t", index=True, mode="a")
